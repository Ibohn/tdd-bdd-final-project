...
def list_products():
    """List all Products (optionally filtered by name, category, available)"""
    ...
    if category:
        products = Product.find_by_category(Category[category]).all()
        return jsonify([p.serialize() for p in products]), status.HTTP_200_OK

    if available is not None:
        avail = str(available).lower() == "true"
        products = Product.find_by_availability(avail).all()
        return jsonify([p.serialize() for p in products]), status.HTTP_200_OK
